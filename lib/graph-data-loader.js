...
function getTopScorer(players) {
  ...
  const series = [
    {name: 'Keren gescoord', data: data.map(obj => obj.serieGoals)},
    {name: 'Keren geassists', data: data.map(obj => obj.serieAssists)}
  ]
...
function getTopAttendeesTraining(players) {
  ...
  const labels = data.map(obj => obj.label)
  const series = [{
    name: 'Aantal keer op training', data: data.map(obj => obj.serie)
  }]
